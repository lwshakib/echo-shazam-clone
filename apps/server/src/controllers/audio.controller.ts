...
export const identifyAudioHandler = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (result) {
      return res
        .status(200)
        .json(
          new ApiResponse(200, { match: true, song: result }, "Match found")
        )
    } else {
      return res
        .status(200)
        .json(new ApiResponse(200, { match: false }, "No match found"))
    }
  }
)
